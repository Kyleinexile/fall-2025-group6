...
if st.button("🚀 Extract KSAs", type="primary", disabled=not can_run, use_container_width=True):
    try:
        ...
        # CRITICAL: Clear ALL provider keys first to prevent fallback to Streamlit secrets
        # This ensures only the user's selected provider key is available
        ...
        # Restore ALL provider keys (including Streamlit secrets)
